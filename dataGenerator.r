...
SP$addTraitADE(nQtlPerChr = sapply(qtl_pos, length),
              ...
              meanDD = c(1,1),# 0 0.2 0.5 1
              varDD = c(0.5,0.5),
              relAA = 1,#0 0.2 0.5 1
              corA = matrix(c(1,0.3,0.3,1),nrow = 2))

#convert the observed heritablity of SR into liability scale
# Converth2 = function(h2,prevalence){
#   return(h2*((1-prevalence)*prevalence)/(dnorm(qnorm(1-prevalence))^2))
# }
# c(0.28,0.13,0.14) %>% mean()
# Converth2(h2 = 0.28,prevalence = 0.6227)
# Converth2(h2 = 0.13,prevalence = 0.6769)
# Converth2(h2 = 0.14,prevalence = 0.7447)
# h2 ranged from 0.13 to 0.28 in both observed and liability scale, taking 0.17 as the h2 for SR
#
