...
calEBV = function(x,y,h2,type = "D"){
  ...
  calAUC = function(auc_dt){
    setorder(auc_dt,-risk)
    ind_diseased = sum(auc_dt$status == 0)
    ind_not_diseased = sum(auc_dt$status == 1)
    rank_mean = mean(which(auc_dt$status == 0))
    AUC = (1/ind_not_diseased) * (rank_mean - (ind_diseased/2) - (1/2) )
    return(AUC)
  }

  #   calAUC2 = function(auc_dt){
  #   setorder(auc_dt,risk)
  #   ind_diseased = sum(auc_dt$status == 1)
  #   ind_not_diseased = sum(auc_dt$status == 0)
  #   rank_mean = mean(which(auc_dt$status == 1))
  #   AUC = (1/ind_not_diseased) * (rank_mean - (ind_diseased/2) - (1/2) )
  #   return(AUC)
  # }
...
if(!is.null(a_dt)){
  ...
  auc_dt = data.table(risk = ebv1,status = GV1)

  acc  = cor(GV1,ebv1)

  acc2  = cor(phe_test$Days_to_death,(1-ebv1))

  AUC = calAUC(auc_dt = auc_dt)

  # AUC = calAUC2(auc_dt = auc_dt)

  output = data.table(mname = model_name, ac = acc,ac2 = acc2,auc = AUC,accov = cov(GV1,ebv1),va = var(ebv1),vg = var(GV1))
